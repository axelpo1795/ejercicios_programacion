    ...
    try:
        with open(file_path, 'w', encoding='utf-8', newline='') as file:
            writer = csv.DictWriter(
                file,
                headers,
                delimiter='\t'   # Separador por tabulaciones
            )
            writer.writeheader()
            writer.writerows(data)

        print("Archivo TSV creado correctamente.")
    except Exception as error:
        print("Ocurrió un error al escribir el archivo TSV:", error)
...
def fill_games_list():
    ...
    try:
        write_tsv_file('video_games.tsv', games_list, game_headers)
    except Exception as error:
